...
describe('Highlight Utilities', () => {
  ...
  it('should get highlights for year', () => {
    ...
    ['enkutatash', 'meskel', 'ketera', 'timkat', 'genna', 'adwa_e', 'patriots_e', 'derg_e'].forEach(id => { expect(ethIds.has(id)).toBe(true); });
    ...
    ['g_new_year', 'genna_g', 'adwa', 'labour', 'patriots', 'derg', 'meskel_g', 'christmas'].forEach(id => { expect(gregIds.has(id)).toBe(true); });
